...
def build_daily_plan_messages(input_data: Dict[str, Any]) -> List[Dict[str, str]]:
    ...
    user = (
        ...
        "- title: 30자 이하, 반드시 구체적 대상(에어컨·커튼·창문·선풍기·환기·타이머 등)과 동작을 담은 행동형으로 쓴다(예 '아침 환기로 냉방 시작 늦추기', '오후 에어컨 1도 올리기').\n"
        "- title·action에 '탄소 절약 시작하기', '추천 행동 실천하기', '절약해 보기', '절약하세요' 같은 추상적·구호성·메타 표현은 절대 쓰지 않는다. 모든 항목은 그 자체로 바로 실행 가능한 구체적 절전 동작이어야 한다.\n"
        "- action: 120자 이하, 실제로 따라 할 수 있는 구체적 단계로 적는다(추상적 권유 금지).\n"
